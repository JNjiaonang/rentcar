...
CreateThread(function()
    Wait(1000)
    Wrapper:Blip('RentCarBlip','租车',Config.Settings.RentLocation,811,3)
    Wrapper:Target('bbv-rentacar-target','租车',vec3(Config.Settings.RentLocation.x,Config.Settings.RentLocation.y,Config.Settings.RentLocation.z + 1),'bbv-rentacar')
...
RegisterNetEvent('bbv-rentacar',function()
    if Main.RentCar ~= nil and Config.Settings.LimitCars then Wrapper:Notify("你已经租了一辆车.") return end
    RentACar()
end)
...
RegisterNUICallback('PreviewInformation', function(data)
    ...
    Wrapper.TriggerCallback('bbv-rentacar:hasmoney', function(money)
        if money then 
            Wrapper:Notify("你租用 " .. car .. ' 付款 ' .. price .. "$")
            Main:SpawnVeh(carspawn,car,true,false)
        else
            Wrapper:Notify("你没有足够的钱!")
        end
    end,price)
end)
...
function Main:SpawnVeh(coords,model,isnetworked,test)
    ...
    Wrapper:Blip('ReturnCar','退还租金',Config.Settings.ReturnLocation,812,53)
    ...
    if Config.Settings.CarBlip then
        ...
        AddTextComponentSubstringPlayerName('租车')
...
RegisterNetEvent('bbv-renttimer',function()
    ...
    while Main.TestDrive do 
        ...
        Wrapper:Prompt('剩余时间 ' ..Config.Settings.TestDriveTime - msg)
